...
public class AdminDAO {
    ...
    public long totalPrice() {
        long total = 0;
        try {
            String query = "select  SUM((c.Amount * p.Price)*5/100)\n" +
                            "from  Cart c,  Product p, Orders od\n" +
                            "where c.ProductID = p.ProductID and od.CartID = c.CartID and od.PaymentStatus = 3";
            conn = new DBContext().getConnection();
            ps = conn.prepareStatement(query);
            rs = ps.executeQuery();
            while (rs.next()) {
